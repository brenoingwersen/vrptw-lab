...
class Variables:
    ...
    def __init__(
        ...
        self._node_load_vars = np.array(
            [
                model.new_int_var(
                    0, instance.constraints_config.truck_capacity, f"load_{idx}"
                )
        ...
        logger.info(
            f"Model variables created: nodes={instance.n_nodes}, arcs={len(self._arcs)}"
        )
    ...
    def arc_distance(self) -> np.ndarray:
        ...
        return np.array(
            [
                self.instance.travel_time(int(i), int(j))
    ...
    def iter_load_arcs(
        ...
        for (node_from, node_to), arc_var in self.iter_arcs():
            yield (
                ...
                arc_var,
                int(self.instance.demand[node_to]),
            )
